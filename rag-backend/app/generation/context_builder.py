...
def select_generation_chunks(
    ...
    if not explicit_refs:
        ...
        # Pass 1: Multi-tier authority representation
    ...
    for c in chunks:
        ...
        if meta.get("section"):
            sec_val = str(meta["section"]).strip()
            chunk_provs.extend([f"SEC_{sec_val}", f"CGST_SEC_{sec_val}", f"IGST_SEC_{sec_val}", f"UNKNOWN_SEC_{sec_val}"])
        if meta.get("rule"):
            rul_val = str(meta["rule"]).strip()
            chunk_provs.extend([f"RUL_{rul_val}", f"CGST_RUL_{rul_val}", f"IGST_RUL_{rul_val}"])
        ...
        # 2. Match available chunk references
        if not is_direct:
            c_refs = ReferenceResolver.extract_available_chunk_references([c])
            c_keys = {r.canonical_key for r in c_refs} | {r.base_key for r in c_refs}
            if any(req_k in c_keys for req_k in req_canonical_keys) or any(req_b in c_keys for req_b in req_base_keys):
                is_direct = True

        # 3. Match circular number in document path / title
    ...
    # Sort direct evidence to prioritize chunks with specific requested subsection/clause operative text
    def _direct_evidence_sort_key(c: Dict[str, Any]):
        text = (c.get("text") or c.get("content") or "").lower()
        meta = c.get("metadata") or {}
        sec_label = (meta.get("section_label") or "").lower()
        rel_lower = (_rel(c) or "").replace("\\", "/").lower()

        has_sub_match = False
        is_primary_target = False
        is_cross_ref_target = False

        for r in explicit_refs:
            sub_clauses = re.findall(r'\(([0-9a-zA-Z]+)\)', r.canonical_key)
            if sub_clauses and any(f"({s.lower()})" in text for s in sub_clauses):
                has_sub_match = True
            anchor_match = (
                str(c.get("_pinned_canonical_key") or "") in (r.canonical_key, r.base_key)
                or str(c.get("_anchor_provision") or "") in (r.canonical_key, r.base_key)
            )
            if anchor_match and "act" in rel_lower and r.ref_type == "SECTION":
                is_primary_target = True
            elif anchor_match and "rule" in rel_lower and r.ref_type == "RULE":
                is_primary_target = True

            if r.provision_num:
                pnum = r.provision_num.lower()
                if r.ref_type == "SECTION":
                    if f"section {pnum}" in sec_label or ("act" in rel_lower and (f"section - {pnum}" in text or f"section {pnum}." in text or f"{pnum}. (1)" in text)):
                        is_primary_target = True
                    elif f"section {pnum}" in text:
                        is_cross_ref_target = True
                elif r.ref_type == "RULE":
                    if f"rule {pnum}" in sec_label or ("rule" in rel_lower and (f"rule - {pnum}" in text or f"rule {pnum}." in text)):
                        is_primary_target = True
                    elif f"rule {pnum}" in text:
                        is_cross_ref_target = True

        if is_primary_target and has_sub_match:
            prio = 0  # Exact primary statute section + requested subsection/clause
        elif is_primary_target:
            prio = 1  # Exact primary statute section
        elif is_cross_ref_target and has_sub_match:
            prio = 2  # Cross-reference with requested subsection
        elif is_cross_ref_target:
            prio = 3  # Cross-reference
        else:
            prio = 4  # Other sibling chunk from same document
        return (prio, 0 if len(text.strip()) >= 250 else 1, -len(text.strip()))

    direct_evidence.sort(key=_direct_evidence_sort_key)

    # Add direct evidence up to budget
    for c in direct_evidence:
        _try_add(c, max_per_doc=6)

    # Pass 3: Identify connected statutory evidence
    ...
    # Pass 4: Fill remaining token budget
    for c in chunks:
        tier = _get_authority_tier(c)
...
def build_canonical_authority_registry(chunks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for i, c in enumerate(chunks):
        ...
        anchor_p = str(c.get("_anchor_provision") or c.get("_pinned_canonical_key") or "")
        sec_lbl = str((c.get("metadata") or {}).get("section_label") or "").lower()

        def _ref_pri(r):
            if anchor_p and (r.canonical_key == anchor_p or r.base_key == anchor_p):
                return 0
            if sec_lbl and r.provision_num and f"section {r.provision_num.lower()}" in sec_lbl:
                return 1
            if sec_lbl and r.provision_num and f"rule {r.provision_num.lower()}" in sec_lbl:
                return 1
            if not r.is_cross_ref:
                return 2
            return 3

        refs.sort(key=_ref_pri)
...
def format_allowed_authorities_block(registry: List[Dict[str, Any]]) -> str:
    ...
    for item in registry:
        ...
        keys_str = f" ({', '.join(item['canonical_keys'][:4])})" if item.get("canonical_keys") else ""
        span = item["support_span"].replace("\n", " ").strip()
        lines.append(f"{marker} [{tier}]{keys_str} {disp} | Source: {doc}")
        if span:
