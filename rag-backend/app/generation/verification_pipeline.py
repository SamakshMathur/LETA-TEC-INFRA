...
def verify_substantive_claim_support(
    ...
    3. Marker Laundering Prevention (Fix A): Verify that cited authorities in the claim are substantively
    ...
    4. Direct vs Cross-Reference Verification (Fix B): If an authority appears in evidence only as an
    ...
    5. Contradiction & Unrestricted Entitlement Check (Fix C): Detect assertions of unrestricted entitlement
    ...
    for m, chunk in marker_chunks:
        ...
        # FIX C: Contradiction and Unrestricted Entitlement Check
    ...
    # FIX A: Marker Laundering Prevention
    # Verify that every specific legal authority asserted in this claim is supported
    # by at least one of the cited (S#) markers.
    ...
    # FIX B: Cross-Reference vs Direct Authority Verification
    # If the claim asserts substantive operational rules for an authority that only appears as a cross-reference
    # in evidence without any primary chunk directly dedicated to that authority
    ...
    # Semantic proof requirement: when proposition-level support cannot be established reliably,
    # the claim must be treated as unsupported.
    is_supported = (len(issues) == 0) and statute_match

    # If markers are cited but none of the marker chunks exist in the map, or if references exist with no support in available refs
...
async def run_verification_pipeline(
    *,
    ...
    # Decompose answer into claim units (bullets vs paragraph sentences) to preserve list structure
    evidence_ledger: list[dict[str, Any]] = []
    lines = answer.split("\n")
    claim_units: list[tuple[str, str]] = []  # (type: 'bullet'|'sentence'|'blank', text)
    ...
    if needs_remediation:
        # Check strict safe REPAIR conditions:
        # Condition A: Primary requested authority must be present and supported in evidence.
        primary_supported = (
            all(
        ...
        if primary_supported:
            ...
            for utype, utext in claim_units:
                ...
                if not has_unsupported and chk["supported"]:
                    ...
                    # Check if this unit contains a requested primary reference
                    has_primary = (
                        any(
                    ...
                    if not has_primary:
                        # Safe to drop peripheral sentence or bullet that contains unsupported citations
                        continue
                    else:
                        # Mixed primary + unsupported peripheral reference: strip unsupported citation phrases
                        cleaned_unit = utext
                        for sr in u_refs:
            ...
            # Re-audit repaired candidate text
            if candidate_text:
