...
def build_vector_store(chunks_path: Path, index_path: Path):
    ...
    if embeddings.ndim != 2 or embeddings.shape[1] != VECTOR_DIM:
        raise ValueError(f"Embedding dimension {embeddings.shape} != configured VECTOR_DIM {VECTOR_DIM}")

    # Vectors are normalized, so inner product is cosine similarity.
