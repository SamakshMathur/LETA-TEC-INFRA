...
def extract_text_from_scanned_pdf(path):
    ...
    for idx, image in enumerate(images):
        ...
        stripped = text.strip()
        if not stripped:
            # Skip entirely — the sentinel "[OCR_EMPTY_PAGE]" used to be stored
            # here and embedded as if it were real content, polluting the retrieval
            # pool with a garbage vector.  An empty OCR result means nothing usable
            # was found on this page; the right action is to omit it.
            continue
        pages.append({
            "text": stripped,
            "metadata": {
