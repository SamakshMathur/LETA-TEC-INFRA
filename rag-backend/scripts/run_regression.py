...
def run_tests(
    ...
    print(f"Running {len(filtered)} tests  [top_k={top_k}]")
    print("─" * 70)
...
def print_summary(results: list[dict], min_coverage: int) -> bool:
    ...
    if ok:
        ...
        # Print the failing high-priority tests for quick triage
...
def main():
    ...
    print("Initialising retriever (may take 30–60s for sub-index builds)…")
    try:
