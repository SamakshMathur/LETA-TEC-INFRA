...
export const BackgroundBeams = React.memo(({ className = "", maskCenter = false }) => {
  return (
    <div
      className={`pointer-events-none absolute inset-0 h-full w-full ${className}`}
      style={maskCenter ? {
        // Radial mask: fully transparent in the centre (where content is),
        // fully visible at the edges — beams never overlap the hero text/CTAs
        WebkitMaskImage: 'radial-gradient(ellipse 55% 60% at 50% 45%, transparent 0%, transparent 35%, black 70%)',
        maskImage:       'radial-gradient(ellipse 55% 60% at 50% 45%, transparent 0%, transparent 35%, black 70%)',
      } : undefined}
    >
      <svg
        aria-hidden="true"
        ...
        <defs>
          {pathData.map((_, i) => (
            ...
            >
              ...
              <stop offset="50%"  stopColor="#4FB7C5" stopOpacity="1" />
